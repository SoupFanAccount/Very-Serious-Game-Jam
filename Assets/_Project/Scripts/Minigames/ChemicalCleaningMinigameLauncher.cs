...
{
    /// <summary>
    /// Concrete <see cref="LaunderingMinigameLauncher"/> that opens the chemical cleaning minigame. Assign it
    ...
    /// </summary>
    public class ChemicalCleaningMinigameLauncher : LaunderingMinigameLauncher
    {
        ...
        [SerializeField] private ChemicalCleaningMinigameController controller;
        ...
        public override void StartLaundering(int dirtyMoneyAvailable, PlayerInteractionController interactor)
        {
            if (controller == null)
            {
                Debug.LogError($"{nameof(ChemicalCleaningMinigameLauncher)}: no controller assigned.", this);
                return;
            }
        ...
        private void HandleSessionFinished(ChemicalCleaningResult result)
        {
            controller.SessionFinished -= HandleSessionFinished;
