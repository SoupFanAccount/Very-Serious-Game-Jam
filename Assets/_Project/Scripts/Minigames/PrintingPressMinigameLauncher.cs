...
{
    /// <summary>
    /// Concrete <see cref="LaunderingMinigameLauncher"/> that opens the printing-press minigame. Assign it to a
    /// <see cref="WashingStationInteractable"/>. It shows the minigame panel, freezes the triggering player,
    /// starts a session and, when the session ends, hides the panel, restores the player and relays the result
    /// back to them.
    /// </summary>
    public class PrintingPressMinigameLauncher : LaunderingMinigameLauncher
    {
        ...
        [SerializeField] private PrintingPressMinigameController controller;
        ...
        public override void StartLaundering(int dirtyMoneyAvailable, PlayerInteractionController interactor)
        {
            if (controller == null)
            {
                Debug.LogError($"{nameof(PrintingPressMinigameLauncher)}: no controller assigned.", this);
                return;
            }
        ...
        private void HandleSessionFinished(PrintingPressResult result)
        {
            controller.SessionFinished -= HandleSessionFinished;
